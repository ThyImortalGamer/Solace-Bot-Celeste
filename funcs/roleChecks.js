...
module.exports = {
    run(client) {
        ...
        database.query("SELECT * FROM premium", [], (err, rows, fields) => {
            ...
            rows.forEach(row=>{
                ...
                if(!userRoles.includes(premiumRole)&&row.has == "1") {
                    member.addRole(premiumRole)
                } else if(userRoles.includes(premiumRole)&&row.has != "1") {
        ...
        database.query("SELECT * FROM members", [], (err, rows, fields) => {
            ...
            rows.forEach(row=>{
                ...
                if(!userRoles.includes(memberRole)&&row.has == "1") {
                    member.addRole(memberRole)
                } else if(userRoles.includes(memberRole)&&row.has != "1") {
    ...
    init() {
        ...
        database.query("CREATE TABLE IF NOT EXISTS premium (userId VARCHAR(19) PRIMARY KEY, has TEXT)", [], (err, rows, fields) => {
            if (err) return console.log("Error on premium table creation:\n" + err)
        });
        database.query("CREATE TABLE IF NOT EXISTS members (userId VARCHAR(19) PRIMARY KEY, has TEXT)", [], (err, rows, fields) => {
