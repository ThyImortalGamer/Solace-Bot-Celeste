...
module.exports = {
    ...
    run(msg) {
        ...
        database.query(`SELECT * FROM spamScores WHERE userId ="${msg.author.id}"`, [], (err, rows, fields) => {
            if (rows < 1) {
                database.query("REPLACE INTO spamScores (userId, points) VALUES (?, ?)", [msg.author.id, 1], (err, rows, fields) => {
                    if (err) return console.log("Error on replace into spamScores:\n"+err)
                });
            } else {
                rows.forEach(row => {
                    database.query(`UPDATE spamScores SET points = ${row.points + 1} WHERE userId = ${msg.author.id}`);
    ...
    init() {
        database.query("CREATE TABLE IF NOT EXISTS spamScores (userId VARCHAR(19) PRIMARY KEY, points INTEGER)", [], (err, rows, fields) => {
