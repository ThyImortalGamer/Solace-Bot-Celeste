...
module.exports = class extends Command {
    ...
    async run(msg, [...params]) {
        ...
        if (!this.client.guilds.get('383660119218585600').member(msg.author.id)) return msg.reply(`You must be a member of the ${this.client.guilds.get('383660119218585600').name} guild to use this command.`);
                ...
                if (msg.content.startsWith('Cel, ')) {
                    vent = msg.content.substring('Cel, staffvent '.length)
                } else if (msg.content.startsWith('Celeste, ')) {
                    vent = msg.content.substring('Celeste, staffvent '.length)
                } else if (msg.content.startsWith('!')) {
                    vent = msg.content.substring('!staffvent '.length)
                }
        
        database.query(`SELECT * FROM staffAnonIds WHERE userId="${msg.author.id}"`, [], (err, rows, fields) => {
            ...
            if (rows.length >= 1) {
                ...
                database.query("REPLACE INTO staffAnonIds (userId) VALUES (?)", [msg.author.id], (err, rows, fields) => {
    ...
    async init() {
        ...
        database.query("CREATE TABLE IF NOT EXISTS staffVents (userId TEXT, vent TEXT, date TEXT)", [], (err, rows, fields) => {
            if (err) console.log("Error on vent table creation:\n" + err)
        });
        database.query("CREATE TABLE IF NOT EXISTS staffAnonIds (userId VARCHAR(19) PRIMARY KEY, id INT UNSIGNED NOT NULL AUTO_INCREMENT UNIQUE KEY)", [], (err, rows, fields) => {
