...
module.exports = class extends Monitor {
            ...
            database.query(`SELECT * FROM profiles WHERE userId =${msg.author.id}`, [], (err, rows, fields) => {
                ...
                rows.forEach(row => {
                    ...
                    if (row.discipline == 'No') {
                        database.query(`REPLACE INTO profiles (userId, username, points, level, credits, colour, discipline) VALUES (?, ?, ?, ?, ?, ?, ?)`, [row.userId, row.username, row.points + 2, row.level, row.credits + 5, row.colour, row.discipline], (err, row, fields) => {
                            if (err) console.log('Error on points (no discipline)' + err)
                        });
                    } else if (row.discipline == 'Yes') {
                        database.query(`REPLACE INTO profiles (userId, username, points, level, credits, colour, discipline) VALUES (?, ?, ?, ?, ?, ?, ?)`, [row.userId, row.username, row.points + 1, row.level, row.credits + 1, row.colour, row.discipline], (err, row, fields) => {
    ...
    async init() {
        ...
        console.log('level.js has init\'d')
    }
